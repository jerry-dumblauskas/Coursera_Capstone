...
This is an analysis report for the Coursera Data Science Capstone project. The goal of this analysis is to create a predictive text model using a large text repository of documents as training data. Basic R techniques will be used to perform the analysis and build the groundwork for an eventual predictive model.
...
freq1 <- get_freq(removeSparseTerms(TermDocumentMatrix(corpora), 0.99))
freq2 <- get_freq(removeSparseTerms(TermDocumentMatrix(corpora, control = list(tokenize = bigram)), 0.99))
freq3 <- get_freq(removeSparseTerms(TermDocumentMatrix(corpora, control = list(tokenize = trigram)), 0.99))
```

and let's look at the graphs
```{r}
make_plot(freq3, paste(datapoints ,"Highest Number of Triples in the Sample set"))
make_plot(freq2, paste(datapoints ,"Highest Number of Doubles in the Sample set"))
make_plot(freq1, paste(datapoints ,"Highest Number of Singles in the Sample set"))
```
